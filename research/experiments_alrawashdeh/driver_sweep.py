...
def run_config(v, a, j, s, tag):
    controller = rawashdeh_ss_sim.StepperController(dies, die_l, v, a, j, s, alpha)
    die_grid_xml = ""
    for cx, cy in dies:
        die_grid_xml += (
    ...
    xml = rawashdeh_ss_sim.get_model_xml(die_l, die_grid_xml)
    model = mujoco.MjModel.from_xml_string(xml)
    data = mujoco.MjData(model)
    cancel_reactions = rawashdeh_ss_sim.make_reaction_canceller(model)
