...
the real full MuJoCo model (research/simulation/litho_sim.py) via
...
# litho_control._run only returns per-die SCANNING segments and discards
...
model, data = lc.build(DIES, DIE_L)
...
while data.time < lc.HOLD_TIME:
    data.ctrl[lc.CTRL["w_ls_x"]] = xw0
    data.ctrl[lc.CTRL["w_ls_y"]] = yw0
    data.ctrl[lc.CTRL["r_ls_x"]] = xr0
    data.ctrl[lc.CTRL["r_ls_y"]] = yr0
    cancel_reactions(data)
    mujoco.mj_step(model, data)
    if data.time > lc.HOLD_TIME - 0.05:
        hold_wy.append(data.body("wafer").xpos[1] - yw0)
        hold_ry.append(data.body("mask").xpos[1] - yr0)
        hold_wx.append(data.body("wafer").xpos[0] - xw0)
...
while data.time < max_time:
    ...
    data.ctrl[lc.CTRL["w_ls_x"]] = xw
    data.ctrl[lc.CTRL["w_ls_y"]] = yw
    data.ctrl[lc.CTRL["r_ls_x"]] = xr
    data.ctrl[lc.CTRL["r_ls_y"]] = yr
    data.ctrl[lc.CTRL["w_ls_x_v"]] = vxw
    data.ctrl[lc.CTRL["w_ls_y_v"]] = vw
    data.ctrl[lc.CTRL["r_ls_x_v"]] = 0.0
    data.ctrl[lc.CTRL["r_ls_y_v"]] = vr

    wafer_y = data.body("wafer").xpos[1]
    mask_y = data.body("mask").xpos[1]
    e_w = wafer_y - off_wy - yw
    e_r = mask_y - off_ry - yr
    e_wx = data.body("wafer").xpos[0] - off_wx - xw

    t_l.append(t)
    ywref_l.append(yw); xwref_l.append(xw); yrref_l.append(yr)
    ew_l.append(e_w); er_l.append(e_r); ewx_l.append(e_wx)
    wstage_l.append(data.body("w_stage").xpos[1])
    rstage_l.append(data.body("r_stage").xpos[1])
