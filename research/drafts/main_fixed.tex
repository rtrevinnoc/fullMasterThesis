...
where $K_{1,xx}$, $K_{1,xy}$, etc.\ denote the individual components of the $2\times2$ translational stiffness matrix ${}^0\bar{K}^f_1$. Notice that the translational rows (4 and 5) and the rotational row (6) are structurally decoupled within this linear state-space formulation: the translational stiffness and damping entries are zero in the rotational column (column 3), and conversely, the rotational entries are zero in the translational columns (columns 1 and 2). This block-diagonal decoupling directly reflects the parallel spring-damper assumption of the Kelvin-Voigt joint model restricted to planar motion. Nonlinear coupling effects---such as Coriolis and centripetal forces from \eqref{eq:acc_expanded}---are inherently excluded from this linear representation and must be treated as external nonlinear perturbations if a higher-fidelity model is required.

\subsubsection{General State-Space Form for the Chain Bodies}

To systematically construct the global state-space matrices, we must reframe the equations of motion for the chain bodies in terms of the state vector variables. We map the physical planar coordinates of the $p$-th sequential body to the local state vectors: translational position $\mathbf{x}_{p}^f = [x_{6p-5},\, x_{6p-4}]^T$, translational velocity $\mathbf{x}_{p}^{\dot{f}} = [x_{6p-2},\, x_{6p-1}]^T$, rotational position $x_{p}^{\theta} = x_{6p-3}$, and rotational velocity $x_{p}^{\dot{\theta}} = x_{6p}$.
...
For the \textbf{first body in each chain} ($i=2$, index $p$, upstream index $1$), the translational state derivatives ($\dot{\mathbf{x}}_{p}^{\dot{f}} = \ddot{f}_{2_j}$) and rotational state derivatives ($\dot{x}_{p}^{\dot{\theta}} = \ddot{\theta}_{z_{2_j}}$) take the form:
\begin{align}
    \dot{\mathbf{x}}_{p}^{\dot{f}} &= 
    \frac{1}{\bar{m}_{2_j}} \Big[ u^0_{2_j} - {}^0\bar{C}^f_{2_j}\mathbf{x}_{p}^{\dot{f}} - {}^0\bar{K}^f_{2_j}\mathbf{x}_{p}^f + {}^0\bar{C}^f_{3_j}\mathbf{x}_{p+1}^{\dot{f}} + {}^0\bar{K}^f_{3_j}\mathbf{x}_{p+1}^f \Big] \notag \\
    &\quad - \frac{1}{m_0} \Big[ u^0_1 - {}^0\bar{C}^f_1\mathbf{x}_{1}^{\dot{f}} - {}^0\bar{K}^f_1 \mathbf{x}_{1}^f + \sum_{k=1}^3 \left( {}^0\bar{C}^f_{2_k}\mathbf{x}_{p_{2_k}}^{\dot{f}} + {}^0\bar{K}^f_{2_k}\mathbf{x}_{p_{2_k}}^f \right) \Big] \\
    \dot{x}_{p}^{\dot{\theta}} &= 
    \frac{1}{I_{z_{2_j}}} \Big[ \tau^0_{2_j} - {}^0\bar{C}_{2_j}x_{p}^{\dot{\theta}} - {}^0\bar{K}_{2_j}x_{p}^{\theta} + {}^0\bar{C}_{3_j}x_{p+1}^{\dot{\theta}} + {}^0\bar{K}_{3_j}x_{p+1}^{\theta} \Big] \notag \\
    &\quad - \frac{1}{I_{z_0}} \Big[ \tau^0_1 - {}^0\bar{C}_1x_{1}^{\dot{\theta}} - {}^0\bar{K}_1 x_{1}^{\theta} + \sum_{k=1}^3 \left( {}^0\bar{C}_{2_k}x_{p_{2_k}}^{\dot{\theta}} + {}^0\bar{K}_{2_k}x_{p_{2_k}}^{\theta} \right) \Big]
\end{align}
...
\begin{align}
    \dot{\mathbf{x}}_{p}^{\dot{f}} &= 
    \frac{1}{\bar{m}_{i_j}} \Big[ u^0_{i_j} - {}^0\bar{C}^f_{i_j}\mathbf{x}_{p}^{\dot{f}} - {}^0\bar{K}^f_{i_j}\mathbf{x}_{p}^f + {}^0\bar{C}^f_{(i+1)_j}\mathbf{x}_{p+1}^{\dot{f}} + {}^0\bar{K}^f_{(i+1)_j}\mathbf{x}_{p+1}^f \Big] \notag \\
    &\quad - \frac{1}{\bar{m}_{(i-1)_j}} \Big[ u^0_{(i-1)_j} - {}^0\bar{C}^f_{(i-1)_j}\mathbf{x}_{p-1}^{\dot{f}} - {}^0\bar{K}^f_{(i-1)_j}\mathbf{x}_{p-1}^f + {}^0\bar{C}^f_{i_j}\mathbf{x}_{p}^{\dot{f}} + {}^0\bar{K}^f_{i_j}\mathbf{x}_{p}^f \Big] \\
    \dot{x}_{p}^{\dot{\theta}} &= 
    \frac{1}{I_{z_{i_j}}} \Big[ \tau^0_{i_j} - {}^0\bar{c}_{\theta,i_j}x_{p}^{\dot{\theta}} - {}^0\bar{k}_{\theta,i_j}x_{p}^{\theta} + {}^0\bar{c}_{\theta,(i+1)_j}x_{p+1}^{\dot{\theta}} + {}^0\bar{k}_{\theta,(i+1)_j}x_{p+1}^{\theta} \Big] \notag \\
    &\quad - \frac{1}{I_{z_{(i-1)_j}}} \Big[ \tau^0_{(i-1)_j} - {}^0\bar{c}_{\theta,(i-1)_j}x_{p-1}^{\dot{\theta}} - {}^0\bar{k}_{\theta,(i-1)_j}x_{p-1}^{\theta} + {}^0\bar{c}_{\theta,i_j}x_{p}^{\dot{\theta}} + {}^0\bar{k}_{\theta,i_j}x_{p}^{\theta} \Big]
\end{align}
...
\begin{align}
    \dot{\mathbf{x}}_{p}^{\dot{f}} &= 
    \frac{1}{\bar{m}_{{N_j}_j}} \Big[ u^0_{{N_j}_j} - {}^0\bar{C}^f_{{N_j}_j}\mathbf{x}_{p}^{\dot{f}} - {}^0\bar{K}^f_{{N_j}_j}\mathbf{x}_{p}^f \Big] \notag \\
    &\quad - \frac{1}{\bar{m}_{({N_j}-1)_j}} \Big[ u^0_{({N_j}-1)_j} - {}^0\bar{C}^f_{({N_j}-1)_j}\mathbf{x}_{p-1}^{\dot{f}} - {}^0\bar{K}^f_{({N_j}-1)_j}\mathbf{x}_{p-1}^f + {}^0\bar{C}^f_{{N_j}_j}\mathbf{x}_{p}^{\dot{f}} + {}^0\bar{K}^f_{{N_j}_j}\mathbf{x}_{p}^f \Big] \\
    \dot{x}_{p}^{\dot{\theta}} &= 
    \frac{1}{I_{z_{{N_j}_j}}} \Big[ \tau^0_{{N_j}_j} - {}^0\bar{c}_{\theta,{N_j}_j}x_{p}^{\dot{\theta}} - {}^0\bar{k}_{\theta,{N_j}_j}x_{p}^{\theta} \Big] \notag \\
    &\quad - \frac{1}{I_{z_{({N_j}-1)_j}}} \Big[ \tau^0_{({N_j}-1)_j} - {}^0\bar{c}_{\theta,({N_j}-1)_j}x_{p-1}^{\dot{\theta}} - {}^0\bar{k}_{\theta,({N_j}-1)_j}x_{p-1}^{\theta} + {}^0\bar{c}_{\theta,{N_j}_j}x_{p}^{\dot{\theta}} + {}^0\bar{k}_{\theta,{N_j}_j}x_{p}^{\theta} \Big]
\end{align}
